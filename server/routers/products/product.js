const { Product, validateProduct, validateProductExcel } = require('../../models/Products/Product');
  ...
  try {
    ...
    for (const product of products) {
      ...
      const newProductData = new ProductData({
        barcode: barcode ? barcode : '47800' + categor.code.toString() + code.toString(),
        code,
        name,
      ...
      const newPrice = new ProductPrice({
        incomingprice: incomingprice ? Math.round(incomingprice * 1000) / 1000 : 0,
        sellingprice: sellingprice ? Math.round(sellingprice * 1000) / 1000 : 0,
        incomingpriceuzs: incomingpriceuzs ? Math.round(incomingpriceuzs * 1) : 0,
        sellingpriceuzs: sellingpriceuzs ? Math.round(sellingpriceuzs * 1) : 0,
        tradeprice: tradeprice ? Math.round(tradeprice * 1000) / 1000 : 0,
        tradepriceuzs: tradepriceuzs ? Math.round(tradepriceuzs * 1) : 0,
        market,
      });
      await newPrice.save();
    ...
    const productcode = new RegExp('.*' + search ? search.code : '' + '.*', 'i');
    const productname = new RegExp('.*' + search ? search.name : '' + '.*', 'i');
    const productcategory = new RegExp('.*' + search ? search.category : '' + '.*', 'i');
...
module.exports.register = async (req, res) => {
  try {
    ...
    const products = await Product.find({
      ...
      .sort({ code: -1 })
      .select(
        'total market category minimumcount width height metrOfProduct metrIncPriceOfProduct totalMetrOfProduct metrPriceOfProduct',
      )
      .populate(
        'price',
        'incomingprice sellingprice incomingpriceuzs sellingpriceuzs tradeprice tradepriceuzs',
...
module.exports.update = async (req, res) => {
  try {
    ...
    if (product.category._id.toString() === category && code !== product.productdata.code) {
      const check = await ProductData.findOne({ category, code, market });
      if (check) {
        return res.status(400).json({
          message: `Diqqat! ${code} kodli mahsulot avval yaratilgan.`,
    ...
    await ProductPrice.findByIdAndUpdate(priceid, {
      ...
      incomingpriceuzs: Math.round(
        (incomingpriceuzs ? incomingpriceuzs : exchangerate.exchangerate * incomingprice) * 1,
      ),
      sellingpriceuzs: Math.round(
        (sellingpriceuzs ? sellingpriceuzs : exchangerate.exchangerate * sellingprice) * 1,
      ),
    ...
    product.unit = unit;
    product.total = total;
    product.metrOfProduct = metrOfProduct;
    product.totalMetrOfProduct = totalMetrOfProduct;
    product.metrPriceOfProduct = metrPriceOfProduct;
    product.metrIncPriceOfProduct = metrIncPriceOfProduct;
    product.width = width;
      ...
      for (const filialId of marke.filials) {
        ...
        if (filialCategory && filialproductData && filialproduct) {
          await ProductPrice.findByIdAndUpdate(filialproduct.price, {
            ...
            tradeprice: Math.round(tradeprice * 1000) / 1000,
            tradepriceuzs: Math.round(tradepriceuzs * 1),
          });
    ...
    const products = await Product.find({
      ...
      .sort({ code: 1 })
      .select(
        'total market category minimumcount width height metrOfProduct totalMetrOfProduct metrPriceOfProduct',
      )
      .populate(
        'price',
        'incomingprice sellingprice incomingpriceuzs sellingpriceuzs tradeprice tradepriceuzs',
...
module.exports.getAll = async (req, res) => {
  try {
    ...
    const products = await Product.find({
      ...
      .sort({ code: 1 })
      .select(
        'name code unit category producttype brand price total minimumcount width height metrOfProduct totalMetrOfProduct metrPriceOfProduct',
      )
...
module.exports.getProducts = async (req, res) => {
  try {
    ...
    const category = new RegExp('.*' + search ? search.category : '' + '.*', 'i');
...
module.exports.getAllProducttypes = async (req, res) => {
  try {
    ...
    const producttypes = await ProductType.find({ market }).select('name category market');
...
module.exports.getProductExcel = async (req, res) => {
  try {
    ...
    const marke = await Market.findById(market);
    if (!marke) {
      return res.status(401).json({ message: "Diqqat! Do'kon malummotlari topilmadi." });
    }
    const code = new RegExp('.*' + search ? search.code : '' + '.*', 'i');
    const name = new RegExp('.*' + search ? search.name : '' + '.*', 'i');
    const category = new RegExp('.*' + search ? search.category : '' + '.*', 'i');
...
module.exports.getAllCategory = async (req, res) => {
  try {
    ...
    const products = await Product.find({
      ...
      .populate('category', 'name code')
      .populate('unit', 'name')
      .populate('price', 'sellingprice sellingpriceuzs incomingprice incomingpriceuzs')
      .lean();
...
module.exports.getProductsInventory = async (req, res) => {
  try {
    ...
    const productcode = new RegExp('.*' + search ? search.productcode : '' + '.*', 'i');

    const productname = new RegExp('.*' + search ? search.productname : '' + '.*', 'i');
...
module.exports.getproductsale = async (req, res) => {
  try {
    ...
    const products = await Product.find({ market })
      ...
      .populate('productdata', 'name code barcode')
      .populate('price', 'sellingprice incomingprice sellingpriceuzs incomingpriceuzs')
      .populate('category', 'name code')
      .populate('unit', 'name');
