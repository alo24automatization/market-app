...
module.exports.getReport = async (req, res) => {
    try {
        ...
        const sales = await DailySaleConnector.find({
            ...
            .populate({
                path: "products",
                select: "totalprice totalpriceuzs pieces price product isMetr",
                populate: {
                    path: "product",
                    select: "price metrIncPriceOfProduct metrOfProduct totalMetrOfProduct metrPriceOfProduct",
                    populate: {
                        path: "price",
                        select:
        ...
        sales.map((sale) => {
            ...
            sale.products.map((product) => {
                ...
                incomingpriceuzs += roundUzs(
                    product.pieces * (product.isMetr ? product.product.metrIncPriceOfProduct : ((product.price && product.price.incomingpriceuzs) ||
    ...
    try {
        ...
        const saleconnector = await DailySaleConnector.find({
            ...
            .populate({
                path: "products",
                select:
                    "totalprice totalpriceuzs pieces price unitprice unitpriceuzs product createdAt user isMetr",
                populate: {
                    path: "price",
                    select: "incomingprice incomingpriceuzs sellingprice sellingpriceuzs",
                },
        ...
        const profitData = saleconnector
            .map((sale) => {
                ...
                const totalincomingpriceuzs = sale.products.reduce(
                    (prev, item) =>
                        prev +
                        item.pieces * (item.isMetr ? item.product.metrIncPriceOfProduct :
