...
const { validateSaleProduct, SaleProduct } = require('../../models/Sales/SaleProduct');
  ...
  try {
    ...
    const [connector, saleconnectors] = await Promise.all([
      DailySaleConnector.findById(dailysaleconnector._id)
        ...
        .populate({
          path: 'products',
          select: `totalprice isMetr unitprice totalpriceuzs unitpriceuzs pieces forWhat more_parameters1 more_parameters2  priceFromLengthAmout
        lengthAmout
        sizePrice`,
          populate: [
            {
      ...
      SaleConnector.find({ market: market, client: client._id })
    ...
    allProducts.forEach((product) => {
      productsByConnector[product.saleconnector] =
        (productsByConnector[product.saleconnector] || 0) + product.totalpriceuzs;
    });

    const paymentsByConnector = {};
    allPayments.forEach((payment) => {
      paymentsByConnector[payment.saleconnector] =
        (paymentsByConnector[payment.saleconnector] || 0) + payment.paymentuzs;
    });

    const discountsByConnector = {};
    allDiscounts.forEach((discount) => {
      discountsByConnector[discount.saleconnector] =
        (discountsByConnector[discount.saleconnector] || 0) + discount.discountuzs;
    });
...
module.exports.addproducts = async (req, res) => {
  try {
    ...
    const totalpriceuzs = Math.round(
      saleproducts.reduce((summ, saleproduct) => {
        return summ + saleproduct.totalpriceuzs;
      }, 0) * 1,
    );
...
module.exports.getsaleconnectors = async (req, res) => {
  try {
    const { market, countPage, currentPage, startDate, endDate, search, filialId } = req.body;
    ...
    if (seller) {
      saleconnectors = await SaleConnector.find({
        ...
        .populate({
          path: 'products',
          select:
            'totalprice isMetr unitprice totalpriceuzs unitpriceuzs pieces createdAt discount saleproducts product fromFilial',
          options: { sort: { createdAt: -1 } },
          populate: {
            path: 'product',
        ...
        .populate({
          path: 'products',
          select:
            'totalprice isMetr priceFromLengthAmout lengthAmout sizePrice forWhat more_parameters1 more_parameters2 unitprice totalpriceuzs unitpriceuzs pieces createdAt discount saleproducts product fromFilial',
          options: { sort: { createdAt: -1 } },
          populate: {
            path: 'saleproducts',
            select: 'pieces totalprice totalpriceuzs',
        ...
        .then((connectors) => {
          return filter(
            connectors,
            (connector) =>
              ((search.client.length > 0 && connector.client !== null && connector.client) ||
  ...
  try {
    ...
    for (const saleproduct of saleproducts) {
      if (saleproduct.pieces > 0) {
        const { totalprice, unitprice, totalpriceuzs, unitpriceuzs, pieces, product } = saleproduct;
        const { error } = validateSaleProduct({
          totalprice,
          totalpriceuzs,
          unitprice,
...
module.exports.getSellers = async (req, res) => {
  try {
    ...
    if (!marke) {
      return res.status(400).json({
        message: "Diqqat! Foydalanuvchi ro'yxatga olinayotgan do'kon dasturda ro'yxatga olinmagan.",
      });
    }
