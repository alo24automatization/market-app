...
const transferWarhouseProducts = async (products) => {
    for (const product of products) {
        const category = await Category.findOne({
            market: product.filial, code: product.categorycode,
        });
        const productdata = await ProductData.findOne({
            market: product.filial, code: product.product.code, category: category._id,
        });
        const productFilial = await Product.findOne({
            market: product.filial, productdata: productdata._id,
        });
        ...
        const warhouseproduct = new WarhouseProduct({
            market: product.market, filial: product.filial, product,
        });
...
module.exports.register = async (req, res) => {
    try {
        const {
            saleproducts, client, packman, discount, payment, debt, market, user, comment,
        } = req.body;
        ...
        const totalprice = convertToUsd(saleproducts.reduce((summ, saleproduct) => {
            return summ + saleproduct.totalprice;
        }, 0));

        const totalpriceuzs = convertToUzs(saleproducts.reduce((summ, saleproduct) => {
        ...
        for (const saleproduct of saleproducts) {
            ...
            const {error} = validateSaleProduct({
                ...
                product: product._id,
                more_parameters1: {
                    length: length === "" ? 0 : length, size: size === "" ? 0 : size, piece: piece === "" ? 0 : piece,
                },
                priceFromLengthAmout: priceFromLengthAmout === "" ? 0 : priceFromLengthAmout,
        ...
        const dailysaleconnector = new DailySaleConnector({
            user, market, saleconnector: saleconnector._id, comment: comment,
        });
        ...
        if (client.name || client._id) {
            if (client._id) {
                ...
                const newClient = new Client({
                    market, name: client.name, packman, phoneNumber: client.phoneNumber
                });
        ...
        const connector = await DailySaleConnector.findById(dailysaleconnector._id)
            ...
            .populate({
                path: "products", select: `totalprice unitprice totalpriceuzs unitpriceuzs pieces forWhat more_parameters1 more_parameters2  priceFromLengthAmout
        lengthAmout
        sizePrice`, populate: [{
                    path: "product", select: "productdata total", populate: {
                        path: "productdata", select: "code name", options: {sort: {code: 1}},
                    },
                },],
            })
...
module.exports.addproducts = async (req, res) => {
    try {
        const {
            saleconnectorid, saleproducts, client, packman, discount, payment, debt, market, user, comment,
        } = req.body;
        ...
        const totalprice = Math.round(saleproducts.reduce((summ, saleproduct) => {
            return summ + saleproduct.totalprice;
        }, 0) * 10000) / 10000;

        const totalpriceuzs = Math.round(saleproducts.reduce((summ, saleproduct) => {
            return summ + saleproduct.totalpriceuzs;
        }, 0) * 1) / 1;
        ...
        const dailysaleconnector = new DailySaleConnector({
            user, market, saleconnector: saleconnector._id, comment,
        });
        ...
        const connector = await DailySaleConnector.findById(dailysaleconnector._id)
            ...
            .populate({
                path: "products", select: `totalprice unitprice totalpriceuzs unitpriceuzs pieces forWhat more_parameters1 more_parameters2  priceFromLengthAmout
        lengthAmout
        sizePrice`, options: {sort: {created_at: -1}}, populate: {
                    path: "product",
...
const sendMessageToClientAboutHisDebt = async (client, debtUzs, debtDate, userPhone, market) => {
    try {
        ...
        const formatMessage = (name, debt, pay_end_date, market_number, market_name,) => {
        ...
        const response = await axios.get(`https://smsapp.uz/new/services/send.php?key=${market.SMS_API_KEY}&number=${validPhoneNumber}&message=${formatMessage(client.name, debtUzs, debtEndDate.format('DD/MM/YYYY'), userPhone, market.name,)}`);
...
module.exports.getsaleconnectors = async (req, res) => {
    try {
        const {
            market, countPage, currentPage, startDate, endDate, search, filialId,
        } = req.body;
        ...
        const saleconnectors = await SaleConnector.find({
            ...
            .populate("payments", "payment paymentuzs comment totalprice totalpriceuzs createdAt cash cashuzs card carduzs transfer transferuzs")
            .populate("discounts", "discount discountuzs createdAt procient products totalprice totalpriceuzs")
            .populate({path: "client", match: {name: name}, select: "name phoneNumber"})
            .populate("packman", "name")
            .populate("user", "firstname lastname")
            .populate("dailyconnectors", "comment ")
            .limit(countPage).lean()
            .then((connectors) => {
                return filter(connectors, (connector) => ((search.client.length > 0 && connector.client !== null && connector.client) || search.client.length === 0) && connector.products.some((item) => search.product ? item.product.productdata.name === search.product : item.product.productdata.name));
            });
        let totaldebtusd = 0;
        let totaldebtuzs = 0;
        let filteredProductsSale = [];
        for (const connector of saleconnectors) {
            ...
            const filterPayment = connector.payments.filter((payment) => {
                return (new Date(payment.createdAt) > new Date(startDate) && new Date(payment.createdAt) < new Date(endDate));
            });
            const filterDiscount = connector.discounts.filter((discount) => {
                return (new Date(discount.createdAt) > new Date(startDate) && new Date(discount.createdAt) < new Date(endDate));
            });
...
// Step 2: Update each sale connector with the total debt for its client
        ...
        res.status(200).json({
            saleconnectors: filteredProductsSale, count,
        });
    } catch (error) {
        console.log(error.message)
        res.status(400).json({error: "Serverda xatolik yuz berdi..."});
    ...
    try {
        ...
        const saleconnectors = await SaleConnector.find({
            ...
            .populate({
                ...
                populate: {
                    path: "product", select: "productdata", populate: {
                        path: "productdata", select: "name code",
                    },
                },
            })
    ...
    try {
        ...
        for (const saleproduct of saleproducts) {
            if (saleproduct.pieces > 0) {
                const {
                    totalprice, unitprice, totalpriceuzs, unitpriceuzs, pieces, product,
                } = saleproduct;
                const {error} = validateSaleProduct({
                    totalprice, totalpriceuzs, unitprice, unitpriceuzs, pieces, product: product._id,
                });
                ...
                const saleproductprice = await SaleProduct.findById(saleproduct._id).select("price");
        ...
        const dailysaleconnector = new DailySaleConnector({
            user, market, saleconnector: saleconnectorid, comment,
        });
        ...
        const connector = await DailySaleConnector.findById(dailysaleconnector._id)
            ...
            .populate({
                path: "products", select: "totalprice unitprice totalpriceuzs unitpriceuzs pieces", populate: {
                    path: "product",
                    select: "poductdata total category",
...
module.exports.payment = async (req, res) => {
    try {
        ...
        const returnpayment = await Payment.findById(newPayment._id).populate({
            path: "saleconnector", select: "client packman", populate: {path: "client", select: "name"},
        });
...
module.exports.getreportproducts = async (req, res) => {
    try {
        const {market, countPage, currentPage, startDate, endDate, search} = req.body;
        const marke = await Market.findById(market);
        if (!marke) {
            return res.status(400).json({
                message: `Diqqat! Do'kon haqida malumotlar topilmadi!`,
        ...
        const code = new RegExp(".*" + search ? search.codeofproduct : "" + ".*", "i");
        const name = new RegExp(".*" + search ? search.nameofproduct : "" + ".*", "i");
        const client = new RegExp(".*" + search ? search.nameofclient : "" + ".*", "i");
        const firstname = new RegExp(".*" + search ? search.nameofseller : "" + ".*", "i");
        ...
        const saleproducts = await SaleProduct.find({
            ...
            .sort({_id: -1})
            .populate({
                path: "user", select: "firstname lastname", match: {firstname},
            })
            .populate({
                path: "saleconnector",
                select: "id client",
            ...
            .populate({
                path: "product", select: "productdata", populate: {
                    path: "productdata", select: "name code", match: {code, name},
                },
            })
            .populate({
                path: "product", select: "unit", populate: {
                    path: "unit", select: "name",
                },
            })
            .then((saleproducts) => filter(saleproducts, (saleproduct) => search.nameofclient.length > 0 ? saleproduct.product.productdata && saleproduct.product.productdata !== null && saleproduct.user && saleproduct.user !== null && saleproduct.saleconnector && saleproduct.saleconnector.client && saleproduct.saleconnector.client !== null : saleproduct.product.productdata && saleproduct.product.productdata !== null && saleproduct.user && saleproduct.user !== null));
        ...
        res.status(200).send({
            products: saleproducts.splice(countPage * currentPage, countPage), count: count.length,
        });
    } catch (error) {
        res.status(400).json({error: "Serverda xatolik yuz berdi..."});
    }
...
module.exports.chnageComment = async (req, res) => {
    try {
        ...
        res.status(400).json({
            error: "Serverda xatolik yuz berdi...", description: error.message,
        });
    }
};
