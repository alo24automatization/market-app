    ...
    try {
        ...
        allProducts.forEach((product) => {
            productsByConnector[product.saleconnector] =
                (productsByConnector[product.saleconnector] || 0) +
        ...
        allDiscounts.forEach((discount) => {
            discountsByConnector[discount.saleconnector] =
                (discountsByConnector[discount.saleconnector] || 0) +
...
module.exports.getsaleconnectors = async (req, res) => {
    try {
        ...
        for (const connector of saleconnectors) {
            ...
            result.totalsalesprice += filterProducts.reduce(
                (prev, el) => prev + el.totalprice,
                0
            );
            result.totalsalespriceuzs += filterProducts.reduce(
                (prev, el) => prev + el.totalpriceuzs,
                0
            );
            result.discounts += filterDiscount.reduce(
                (prev, el) => prev + el.discount,
                0
            );
            result.discountsuzs += filterDiscount.reduce(
                (prev, el) => prev + el.discountuzs,
                0
            );
            result.payments += filterPayment.reduce(
                (prev, el) => prev + el.payment,
                0
            );
            result.paymentsuzs += filterPayment.reduce(
                (prev, el) => prev + el.paymentuzs,
                0
            );
        }
        ...
        result.debtsuzs =
            result.totalsalespriceuzs - result.paymentsuzs - result.discountsuzs;
